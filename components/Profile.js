...
const Profile = () => {
    ...
    const updateprofile = async () => {
        try {
            setisLoading(true)
            const res = await axios.post('http://localhost:5000/user/updateprofile', { token: localStorage.getItem('e4a'), name: name, country: country })
            if (res.data.success) {
                setshowalert(true)
    ...
    const updatepassword = async () => {
        try {
            setisLoading1(true)
            const res = await axios.post('http://localhost:5000/user/updatepassword', { token: localStorage.getItem('e4a'), password,cpassword,opassword })
            if (res.data.success) {
                setshowalert(true)
                setaletrtmsg(res.data.message)
                setaletrttype('success')
    ...
    return (
        <>
            ...
            {!isLoading && user &&
                <>
                    <Alert sx={{ position: 'fixed', top:showalert? '7%': '-7%', left: '50%', transform: 'translate(-50%,-50%)', opacity: showalert ? 1 : 0, transition: 'opacity 0.4s, top 0.25s', fontSize: { xs: '0.8rem', md: '1rem' }, minWidth: { xs: '80vw', md: 'inherit' }, alignItems: 'center' }} severity={alerttype}>{alertmsg}</Alert>
